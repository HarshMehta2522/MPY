...
const VideoDetail = ({ darkMode }) => {
  ...
  useEffect(() => {
    ...
    // Add event listener to track window width
    const handleResize = () => {
  ...
  // Define the sx property for comments section based on the window width
  const sxForComments = windowWidth >= 768 ? { height: "400px", overflowY: "auto" } : {};
      ...
      <Stack direction={{ xs: "column", md: "row" }}>
        <Box flex={1}>
          <Box sx={{ width: "100%", top: "86px" }}>
            ...
            {commentsExpanded && (
              <Box sx={{ width: "90%", position: "sticky", top: "86px", marginTop: "20px", marginLeft: "25px", ...sxForComments }}>
                <Paper elevation={3} sx={{ backgroundColor: darkMode ? "#000" : "#fff", padding: "10px" }}>
                  <Stack direction="column" gap={2}>
                    ...
                    {windowWidth >= 768 && comments.map((comment) => commentMapping(comment, darkMode))}
                    {windowWidth >= 768 && comments.map((comment) => commentMapping(comment, darkMode))}
                    {windowWidth >= 768 && comments.slice(0, 80).map((comment) => commentMapping(comment, darkMode))}
                    {windowWidth <= 500 && comments.slice(0, 50).map((comment) => commentMapping(comment, darkMode))}
                  </Stack>
                </Paper>
              </Box>
            )}
        ...
        <Box px={2} py={{ md: 1, xs: 5 }} sx={{ height: "1200px", overflowY: "auto" }} justifyContent="center" alignItems="center" className="right-side-feed">
